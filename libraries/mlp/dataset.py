...
class WAVAudioDS(PatchedStrokeDS):
    def __init__(self, files, mk_source, preprocess, patch_width, proc_pool, transform = lambda x:x, nperseg = 256, random_patches = True):
        ...
        freq_data = proc_pool.map(Pipeline(preprocess, nperseg), tqdm(files))#tqdm = progress bar and creates an iterator 
        freq_data = list(filter(lambda x: x is not None, freq_data))#striping empty files ensure empty
        idx_mapping = []
